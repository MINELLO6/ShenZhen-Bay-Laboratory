...
output_folder = 'output_31'
local_moran_folder = 'local_moran_results_31'
histogram_folder = 'histograms_31'
origin_folder = 'origin_31'
global_moran_folder = 'global_moran_31'
best_k_file = 'best_k_values_31.txt'
moran_scatter_folder = 'moran_scatter_31'
...
if not os.path.exists(moran_scatter_folder):
    os.makedirs(moran_scatter_folder)
...
with open(best_k_file, 'w') as k_file:
    ...
    for file_name in tqdm(file_list, desc="Processing files"):
        ...
        plt.figure()
        plt.scatter(data_df['x'], data_df['y'], c='lightgrey', edgecolor='k')
        plt.scatter(data_df['x'][hotspots], data_df['y'][hotspots], c='red', label='Hot spot')
        plt.scatter(data_df['x'][coldspots], data_df['y'][coldspots], c='blue', label='Cold spot')
        plt.scatter(data_df['x'][doughnut], data_df['y'][doughnut], c='lightblue', label='Low-High')
        plt.scatter(data_df['x'][diamond], data_df['y'][diamond], c='pink', label='High-Low')
        plt.legend()
        ...
        # 绘制莫兰散点图
        plt.figure()
        plt.scatter(data_df['z'], moran_loc.y, edgecolor='k')
        plt.axhline(0, color='black', linestyle='--')
        plt.axvline(0, color='black', linestyle='--')
        plt.xlabel('Gene expression')
        plt.ylabel('Spatially lagged gene expression')
        plt.title(f'Moran Scatter Plot for {file_name} with k={best_k}')
        moran_scatter_path = os.path.join(moran_scatter_folder, f'{file_name}_moran_scatter.png')
        plt.savefig(moran_scatter_path, bbox_inches='tight')
        plt.close()
